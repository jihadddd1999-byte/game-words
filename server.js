...
io.on('connection', socket => {
  ...
  // ==========================================
  //    1. استوديو الرسم الجماعي (Canvas)
  // ==========================================
  ...
  // ==========================================
  //    3. نظام الشات والملاحظات اللحظية
  // ==========================================
  socket.on('sendMessage', msg => {
    const player = players.find(p => p.id === socket.id);
    if (!player) return;
    if (player.isMuted) {
      socket.emit('chatMessage', { system: true, message: "⚠️ أنت مكتوم حالياً ولا يمكنك إرسال الرسائل." });
      return;
    }
  ...
  // ==========================================
  //    5. لوحة صلاحيات الأدمن (Kick, Mute, Hide Chat)
  // ==========================================
  ...
  socket.on('admin_toggle_mute', targetId => {
    if (players.length > 0 && socket.id === players[0].id) {
      ...
      if (target) {
        ...
        io.to(target.id).emit('update_mute_status', target.isMuted);
        sendSystemMessage(`تم ${target.isMuted ? 'كتم' : 'إلغاء كتم'} اللاعب ${target.name}.`);
      }
    }
  });

  // إخفاء/إظهار الشات للاعب
  socket.on('admin_toggle_chat_view', targetId => {
    if (players.length > 0 && socket.id === players[0].id) {
      const target = players.find(p => p.id === targetId);
      if (target) {
        target.chatHidden = !target.chatHidden;
        io.to(target.id).emit('update_chat_view_status', target.chatHidden);
        sendSystemMessage(`تم ${target.chatHidden ? 'إخفاء' : 'إظهار'} الشات للاعب ${target.name}.`);
      }
    }
  });

  // ==========================================
  //    6. الخروج والتفقد (Disconnect)
  // ==========================================
...
// لمنع خمول السيرفر على استضافات مثل Render
